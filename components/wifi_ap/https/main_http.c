...
httpd_handle_t start_https_server(bool* changed) {
  ...
  static const httpd_uri_t check_uri = {
      .uri = "/", .method = HTTP_GET, .handler = check, .user_ctx = NULL};

   const httpd_uri_t upload_uri = {.uri = "/upload",
                                         .method = HTTP_POST,
                                         .handler = upload_music_handler,
                                         .user_ctx = (void *) changed};

  /* toooooo much ram
  ESP_LOGI(TAG, "starting the https ..........");
  httpd_ssl_config_t config = HTTPD_SSL_CONFIG_DEFAULT();

  extern const uint8_t servercert_pem_start[]
  asm("_binary_servercert_pem_start"); extern const uint8_t servercert_pem_end[]
  asm("_binary_servercert_pem_end"); extern const uint8_t prvtkey_pem_start[]
  asm("_binary_prvtkey_pem_start"); extern const uint8_t prvtkey_pem_end[]
